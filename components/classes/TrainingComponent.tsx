...
import { TrainingContentContainer, TrainingCopyBlock, UpcomingTrainingBlock, TrainingImageBlock, TrainingTakeawayBlock, TrainingTakeaway } from './Training.styles'
...
const TrainingComponent: React.FC<TrainingSession> = ({_id, name, description, upcoming, picture, price, takeaways, trainingType, slug, cta} : TrainingSession) => {
    const asset = {
    ...
    return (
        <TrainingContentContainer id={slug.current} className='has-bg'>
            ...
            <TrainingImageBlock>
                <SanityComponentImage alt={picture.alt} asset={picture.asset} crop={picture.crop} hotspot={picture.hotspot} width={picture.width} />
            </TrainingImageBlock>
                ...
                <div>
                    {upcoming !== null && upcoming.length > 0 ? (
                        <>
                            ...
                            {upcoming.map(uC => (
                                <p key={uC._key}>{`${formatStartDate(uC.startDate)}`}
                        ...
                        <h2>{`${trainingType === 'group' ? 'Check back for availability' : 'Contact for more information'} `}</h2>
                    )}
                </div>
